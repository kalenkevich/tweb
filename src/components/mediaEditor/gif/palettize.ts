...
import {euclideanDistanceSquared} from './color';
...
export function applyPalette(rgba: any, palette: any, format = 'rgb565'): Uint8Array {
  if(!rgba || !rgba.buffer) {
...
function nearestColorIndexRGBA(r: number, g: number, b: number, a: number, palette: number[][]) {
  let k = 0;
  let mindist = 1e100;
...
function nearestColorIndexRGB(r: number, g: number, b: number, palette: number[][]) {
  let k = 0;
  let mindist = 1e100;
  for(let i = 0; i < palette.length; i++) {
    const px2 = palette[i];
...
export function snapColorsToPalette(palette: number[][], knownColors: number[][], threshold = 5) {
