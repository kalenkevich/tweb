...
function encodeNetscapeExt(stream: any, repeat: number) {
  stream.writeByte(0x21); // extension introducer
...
function encodeColorTable(stream: any, palette: number[][]) {
...
function writeUInt16(stream: any, short: number) {
  stream.writeByte(short & 0xff);
  stream.writeByte((short >> 8) & 0xff);
}

function writeUTFBytes(stream: any, text: string) {
