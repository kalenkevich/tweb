...
export function colorDifferenceYIQSquared(yiqA: number[], yiqB: number[]) {
...
function alpha(array: number[]) {
  return array[3] != null ? array[3] : 0xff;
}

export function colorDifferenceYIQ(yiqA: number[], yiqB: number[]) {
  return Math.sqrt(colorDifferenceYIQSquared(yiqA, yiqB));
}

export function colorDifferenceRGBToYIQSquared(rgb1: number[], rgb2: number[]) {
  const [r1, g1, b1] = rgb1;
  const [r2, g2, b2] = rgb2;
...
export function colorDifferenceRGBToYIQ(rgb1: number[], rgb2: number[]) {
  return Math.sqrt(colorDifferenceRGBToYIQSquared(rgb1, rgb2));
}

export function euclideanDistanceSquared(a: number[], b: number[]) {
  var sum = 0;
  var n;
