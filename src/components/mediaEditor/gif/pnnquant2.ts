...
function find_nn(bins: any, idx: number, hasAlpha: boolean) {
  var nn = 0;
  var err = 1e100;
...
function bin_add_rgb(bin: any, r: number, g: number, b: number) {
  bin.rc += r;
  bin.gc += g;
  bin.bc += b;
  bin.cnt++;
...
export default function quantize(rgba: Uint8Array | Uint8ClampedArray, maxColors: number, opts: any = {}) {
  const {
    format = 'rgb565',
    clearAlpha = true,
