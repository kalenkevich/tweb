          ...
          wholeDialogManager.show(({hide}: ContentRenderProps) => (
            <ImageEditor
              ...
              onSave={async(editedImage: Blob) => {
                ...
                await renderImageFromUrlPromise(newImg, url);
                params.file = newFile;
                params.width = newImg.naturalWidth;
                params.height = newImg.naturalHeight;

                if(newFile.type === 'image/gif') {
                  params.noSound = true;

                  const [duration] = await Promise.all([
                    getGifDuration(newImg),
                    createPosterFromMedia(newImg).then(async(thumb) => {
                      params.thumb = {
                        url: await apiManagerProxy.invoke('createObjectURL', thumb.blob),
                        ...thumb
                      };
                  ...
                  const mimeType = params.file.type as MTMimeType;
                  const scaled = await this.scaleImageForTelegram(newImg, mimeType, true);
                  if(scaled) {
                    params.objectURL = scaled.url;
                    params.scaledBlob = scaled.blob;
                  }
